...
2. 创建或更新主题页（projects 分区或公共层 concepts / entities / operations / decisions）。每条结论写 `来源` 与 `最后核验`：外部仓的文件写成 `<登记名>[@<ref>]:<仓内路径>`；`最后核验` 只在回读来源后才新增一行并写明核验方式，只改措辞、搬迁、修链接不推进日期。矛盾保留并标 `待核验`，悬置项进 `$WIKI/wiki/risks/`。
