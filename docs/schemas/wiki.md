...
两级索引的机械规则：根 `index.md` 只列分区入口；分区（`projects/<项目>/`、`learning/`）各自维护 `index.md`，子索引必须由根 index 直接链接，挂在子索引下的页面即算已入索引，索引不再往下传递。子索引默认是纯导航页：只放标题、紧跟一级标题的一段导语、带链接的列表项和表格，不要求「来源」「最后核验」；一旦写入其它正文（第二段起的段落、没有链接的列表项），就按事实页检查这两节。不强制把正文拆出去，但拆不拆都要有来源。`private/` 下的页面不进根索引，也不算孤儿页；私有页发出的链接不给公共页算入链。
...
- `<登记名>[@<ref>]:<仓内路径>`：外部仓（`config/registry.md` 或 `config/repos.conf` 登记的仓）里的文件，如 `openclaw@v2026.7.1-2:docs/concepts/queue.md`；裸写 `docs/…` 会被当作本仓路径检查。
- 用户提供（YYYY-MM-DD）：口述且无落盘文件时。

## 最后核验

- YYYY-MM-DD（核验方式；未覆盖范围）
```

尚未核验的结论标记 `待核验`。来源互相矛盾时，保留各方表述并标待核验，不得静默覆盖。

「最后核验」只在真正回读来源之后才新增一行，并写明核验方式与这次没覆盖的范围；只改措辞、搬迁或合并页面、修链接、补交叉引用都不新增，日期照旧。

README、根 index、log、月日志不是事实页，不强制这两节；子索引按上文「两级索引的机械规则」判定。
...
机械项由脚本报：断链、未入 index、子索引未入根 index、缺来源 / 核验节、来源路径不存在（外部仓前缀写法不查）、来源节用 `outputs/` 自证、log 格式、根 log 混入条目、孤儿页（私有区豁免）、疑似禁止来源。过期核验、两页冲突仍由模型做。
...
证据缺口和矛盾记入 `wiki/risks/open-questions.md`。lint 默认只出清单——除非用户当轮说「按你的建议改」。脚本能确定的坏链 / 缺节可以修。每次 lint 跑完都追加一条 lint 日志，不论是否改动：第一行机械项数，再按上面六项各写一行「无」或指向 open-questions 的条目；没有日志的语义 lint 等于没做。脚本对「最后核验」只写到月份的页给提示，不计入退出码，随下次回写换成 YYYY-MM-DD，不为消提示空改日期。`sync.sh` 提交前也会跑一次 lint 并打印，只提示不阻断。
